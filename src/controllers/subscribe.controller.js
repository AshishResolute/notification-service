...
export let subscribe = async (req, res, next) => {
  try {
    ...
    if (error)
      return next(
        new ErrorHandler(
          `Invalid Input Provided,check again!`,
          400,
          error.message,
        ),
      );
    ...
    const addUserSubscription = await db.query(
      `insert into subscribers(user_id,event_type,channel,destination) values($1,$2,$3,$4) returning *`,
      [user_id, event_type, channel, destination],
    );
    if (!addUserSubscription.rowCount)
      return next(new ErrorHandler(`Insertion Failed`, 500, "Database Error"));
